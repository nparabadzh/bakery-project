...
func (u UserRepoMysql) FindAll(start, count int) ([]entities.User, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&user.ID, &user.Email, &user.FirstName, &user.LastName, &user.Password, &user.DeliveryAddress, &user.IsAdmin, &user.Created, &user.Modified)
		if err != nil {
			return nil, err
		}
...
func (u *UserRepoMysql) FindByID(id int) (*entities.User, error) {
	...
	err := u.db.QueryRow(statement, id).Scan(&user.ID, &user.Email, &user.FirstName, &user.LastName, &user.Password, &user.DeliveryAddress, &user.IsAdmin, &user.Created, &user.Modified)
	if err != nil {
		return nil, err
	}
	return user, nil
...
func (u *UserRepoMysql) Create(user *entities.User) (*entities.User, error) {
	statement := "INSERT INTO users(email, first_name, last_name, password, delivery_address, isAdmin, created, modified) VALUES( ?, ?, ?, ?, ?, ?, ?,?)"
	result, err := u.db.Exec(statement, user.Email, user.FirstName, user.LastName,
...
func (u UserRepoMysql) FindByEmail(email string) (*entities.User, error) {
	user := &entities.User{}
	statement := "SELECT id, first_name, last_name, password, delivery_address, isAdmin, created, modified FROM users WHERE email=?"
	err := u.db.QueryRow(statement, email).Scan(&user.ID, &user.FirstName, &user.LastName,
		&user.Password, &user.DeliveryAddress, &user.IsAdmin, &user.Created, &user.Modified)
	if err != nil {
		return nil, err
	}
