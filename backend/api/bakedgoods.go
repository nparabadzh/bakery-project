...
func (u *BakedGoodsRepoMySql) FindByID(id int) (*entities.BakedGood, error) {
	...
	err := u.db.QueryRow(statement, id).Scan(&bakedGood.ID, &bakedGood.Name, &bakedGood.PhotoUrl, &bakedGood.Price, &bakedGood.CategoryId)
	if err != nil {
		return nil, err
	}
...
func (u *BakedGoodsRepoMySql) Create(bakedGood *entities.BakedGood) (*entities.BakedGood, error) {
	statement := "INSERT INTO bakedGoods(name, photoUrl, price, category_id) VALUES( ?, ?, ?, ?)"
	result, err := u.db.Exec(statement, bakedGood.Name, &bakedGood.PhotoUrl, &bakedGood.Price, &bakedGood.CategoryId)
	if err != nil {
		return nil, err
	}
...
func (u *BakedGoodsRepoMySql) Update(bakedGood *entities.BakedGood) (*entities.BakedGood, error) {
	statement := "UPDATE bakedGoods SET name=?, photoUrl=?, price=?, category_id=? WHERE id=? "
	_, err := u.db.Exec(statement, bakedGood.Name, &bakedGood.PhotoUrl, &bakedGood.Price, &bakedGood.ID, &bakedGood.CategoryId)
	if err != nil {
		return nil, err
	}
...
func (u BakedGoodsRepoMySql) FindByName(name string) (*entities.BakedGood, error) {
	bakedGood := &entities.BakedGood{}
	statement := "SELECT name, photoUrl, price, category_id FROM bakedGoods WHERE name=?"
	err := u.db.QueryRow(statement, name).Scan(&bakedGood.ID, &bakedGood.Name, &bakedGood.PhotoUrl, &bakedGood.Price, &bakedGood.CategoryId)
	if err != nil {
		return nil, err
	}
	return bakedGood, nil
